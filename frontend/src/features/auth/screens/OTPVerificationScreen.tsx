...
export default function OTPVerificationScreen({ navigation, route }) {
    const { userId, email, type, amount, description, otp: receivedOtp } = route.params || {};
    const [otp, setOtp] = useState('');
    const [isLoading, setIsLoading] = useState(false);

    // Show OTP if received from backend
    React.useEffect(() => {
        if (receivedOtp) {
            Alert.alert('OTP Received', `Your OTP is: ${receivedOtp}`, [{ text: 'OK' }]);
        }
    }, [receivedOtp]);
